...
class StormControlsTest(base_tests.SimpleDataPlane):
    ...
    def runTest(self):
        ...
        response = requests.get(
            URL+"storm/v1", cookies=COOKIES, headers=GET_HEADER)
        ...
        response = requests.post(
            URL+'storm/v1/'+device_id, json=payload, cookies=COOKIES, headers=POST_HEADER)
        assert response.status_code == 200, 'Add storm setting on device_id = '+device_id+'is FAIL'

        # check if add storm setting success
        response = requests.get(URL+'storm/v1/'+device_id,
        ...
        response = requests.delete(
            URL+'storm/v1/'+device_id, cookies=COOKIES, headers=GET_HEADER)
